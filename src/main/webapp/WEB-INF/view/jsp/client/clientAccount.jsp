							...
							<%-- <dl>
								...
								<dd>
									<div>
										...
										<div style="width: 200px;margin: 10px 0; height: 99px;text-align: center;background-color: #e8e8e8;">
											<%
								...
								if(reg.getOfficeAddress() != null)
								{
									%>
										<dl>
											...
											<dd>
												<label><%= reg.getOfficeAddress() %></label>
												
											</dd>
										</dl>
									<%
								}
								if(reg.getAbout() != null)
								{
									%>
										<dl>
											<dt>
									...
									if(reg.getAdmin() != null)
									{
										%>
											<dl>
												...
												<dd>
													<label><%= reg.getAdmin().getOrganizationName() %></label>
													
												</dd>
											</dl>
											<dl>
												<dt>
													<label>Parent Admin Id</label>
												</dt>
												<dd>
													<label><%= reg.getAdmin().getUserid() %></label>
...
<!-- 											<h3 class="success">Password changed successfully !</h3>	 -->
						    	...
						    	else if(status != null && status.equals("notmatched"))
						    	{
						    		%>
						    			<dl>
											<dd>
												<h3  class="error"><%=GeneralConfig.PassMatchErrMsg %></h3>	
											</dd>
										</dl>
						    		<%
						    	}else{
									    ...
									    <%
									    	if(co_users != null && ! co_users.isEmpty())
									    	{
									    		for(Registration user : co_users)
									    		{
									    			%>
										    			<tr>
										    				<td><a href="clientviewuser?uid=<%= user.getUserid()%>"><%= user.getName() %></a></td>
	  ...
	  <button class="profile_status_buttonGen"  onclick="javascript:history.go(-1)"  style="    margin-right: 17px; margin-bottom: 17px;" >
	  <img style="width:23px;height: 23px;border-radius:30px;" alt="Back" src="images/backicon.png">  </button>
</div>
			      </div>
				<%
			}
